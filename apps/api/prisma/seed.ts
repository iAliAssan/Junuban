...
async function main() {
  ...
  const producerDefs = [
    ...
    { slug: "chabahar-fishers-co", name: "تعاونی صیادان چابهار", region: "چابهار، سیستان و بلوچستان", province: "سیستان و بلوچستان", verified: true, bio: "تعاونی صیادان بومی، تامین‌کننده ماهی خشک و دودی به روش سنتی سواحل مکران." },
    { slug: "bushehr-coast-kitchen", name: "آشپزخانه ساحلی بوشهر", region: "بوشهر", province: "بوشهر", verified: true, bio: "تولید ترشی و عسل خانگی به شیوه سنتی بندری، با مواد اولیه محلی بوشهر." },
  ];
  ...
  // 7) قهوه عربی جنوبی — spices category, single producer, two weights.
  await seedProduct({
    slug: "southern-arabic-coffee",
    name: "قهوه عربی جنوبی",
    description: "دانه قهوه عربیکا برشته‌شده با هل، به سبک قهوه‌خانه‌های سنتی جنوب.",
    categorySlug: "spices",
    imageBg: "5c3a21",
    weights: [
      { label: "۲۵۰ گرم", grams: 250 },
    ...
    producers: [
      {
        ...
        isDefault: true,
        onHandGrams: 18000,
        pricesByGrams: { 250: 210000, 500: 395000 },
        skuPrefix: "COF-BND",
      },
    ],
  });

  // 8) عسل کنار — spices category (no dedicated honey category per the
  // locked category set), single producer, two weights, premium pricing.
  await seedProduct({
    slug: "sidr-honey",
    name: "عسل کنار",
    description: "عسل طبیعی کنار، برداشت مستقیم از کندوهای بومی بوشهر، خام و تصفیه‌نشده.",
    categorySlug: "spices",
    imageBg: "a8791e",
    weights: [
      { label: "۵۰۰ گرم", grams: 500 },
    ...
    producers: [
      {
        producerSlug: "bushehr-coast-kitchen",
        isDefault: true,
        onHandGrams: 20000,
        pricesByGrams: { 500: 480000, 1000: 890000 },
        skuPrefix: "HONEY-BSH",
      },
    ],
  });

  // 9) ماهی خشک — spices category, single producer, intentionally LOW
  // stock (exercises the same low-stock UI path as the saffron above,
  // but for a very different product shape).
  await seedProduct({
    slug: "dried-fish",
    name: "ماهی خشک",
    description: "ماهی کولی خشک‌شده به روش سنتی سواحل مکران، بدون هیچ‌گونه افزودنی.",
    categorySlug: "spices",
    imageBg: "4a4438",
    weights: [{ label: "۵۰۰ گرم", grams: 500 }],
    producers: [
      {
        producerSlug: "chabahar-fishers-co",
        isDefault: true,
        onHandGrams: 2000, // only 4 packages left → low-stock in UI
        pricesByGrams: { 500: 175000 },
        skuPrefix: "FISH-CHB",
      },
    ],
  });

  // 10) ترشی بندری — spices category, two producers to exercise the
  // producer-selector UI on a lower-price-point item.
  await seedProduct({
    slug: "bandari-pickle",
    name: "ترشی بندری",
    description: "ترشی مخلوط بندری، تند و معطر، تخمیر شده به روش خانگی جنوب.",
    categorySlug: "spices",
    imageBg: "5a6448",
    weights: [
      { label: "۵۰۰ گرم", grams: 500 },
    ...
    producers: [
      {
        ...
        isDefault: true,
        onHandGrams: 30000,
        pricesByGrams: { 500: 95000, 1000: 175000 },
        skuPrefix: "PIC-BSH",
      },
      {
        producerSlug: "qeshm-herbs",
        onHandGrams: 14000,
        pricesByGrams: { 500: 89000, 1000: 165000 },
        skuPrefix: "PIC-QSHM",
      },
    ],
  });
