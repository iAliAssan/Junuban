...
import { sessionCookieOptions } from "../common/cookies/cookie-options.util";
...
export class CartContextService {
  ...
  async resolveCart(req: Request, res: Response): Promise<Cart> {
    ...
    if (!guestToken) {
      ...
      res.cookie(
        guestCartConfig.cookieName,
        guestToken,
        sessionCookieOptions(guestCartConfig.maxAgeDays * 24 * 60 * 60 * 1000),
      );
    }
