...
describe("CartService", () => {
  ...
  describe("addItem", () => {
    ...
    it("clamps quantity to 99 even if a larger value is requested", async () => {
      ...
      // No packaging supplied -> null path; no existing line (findFirst
      // defaults to null in makeService), so this goes through create.
      await service.addItem("cart-1", { variantId: "v1", quantity: 500 } as never);

      const createArgs = (prisma.cartItem.create as jest.Mock).mock.calls[0][0];
      expect(createArgs.data.quantity).toBe(99);
    ...
    it("merges into an existing identical line (same variant + same packaging) by summing quantity", async () => {
      ...
      // No packaging supplied -> null path -> looked up and updated via
      // findFirst/update, not findUnique/upsert (see cart.service.ts
      // findCartLine/upsertCartLine for why: Prisma's compound-unique
      // `where` can't express a literal `null` for packagingOptionId).
      (prisma.cartItem.findFirst as jest.Mock).mockResolvedValue({ id: "line-1", quantity: 2 });

      await service.addItem("cart-1", { variantId: "v1", quantity: 3 } as never);

      const updateArgs = (prisma.cartItem.update as jest.Mock).mock.calls[0][0];
      expect(updateArgs.where).toEqual({ id: "line-1" });
      expect(updateArgs.data.quantity).toBe(5); // 2 existing + 3 new
      expect(prisma.cartItem.upsert).not.toHaveBeenCalled();
    });
  ...
  describe("mergeGuestCartOnLogin", () => {
    ...
    it("sums quantities when a merged line already exists in the customer cart", async () => {
      ...
      // Guest item has no packaging -> null path -> findFirst/update, not
      // findUnique/upsert (see cart.service.ts findCartLine/upsertCartLine).
      (prisma.cartItem.findFirst as jest.Mock).mockResolvedValue({ id: "line-1", quantity: 4 }); // existing line in customer cart

      await service.mergeGuestCartOnLogin("guest-token", "customer-1");

      const updateArgs = (prisma.cartItem.update as jest.Mock).mock.calls[0][0];
      expect(updateArgs.where).toEqual({ id: "line-1" });
      expect(updateArgs.data.quantity).toBe(6); // 4 existing + 2 from guest cart
