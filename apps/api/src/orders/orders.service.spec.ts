...
describe("OrdersService", () => {
  function makeService(opts: { cartOverrides?: Partial<CartView>; prismaOverrides?: Record<string, unknown> } = {}) {
    ...
    return { service, prisma, cart, addresses, config, paymentProviders, txMock };
  }
  ...
  describe("checkout — address resolution", () => {
    it("uses an inline shippingAddress when no addressId is given", async () => {
      const { service, txMock } = makeService();
      await service.checkout("cart-1", VALID_CHECKOUT_DTO, "customer-1");
      const createArgs = (txMock.order.create as jest.Mock).mock.calls[0][0];
      expect(createArgs.data.shippingAddress.recipientName).toBe("علی محمدی");
    });

    it("resolves a saved address via addressId, ownership-checked through AddressesService", async () => {
      const { service, addresses, txMock } = makeService();
      const dto = {
        paymentMethod: "CARD_TO_CARD" as const,
      ...
      await service.checkout("cart-1", dto, "customer-1");

      expect(addresses.getOwned).toHaveBeenCalledWith("customer-1", "addr-1");
      const createArgs = (txMock.order.create as jest.Mock).mock.calls[0][0];
      expect(createArgs.data.shippingAddress.recipientName).toBe("علی محمدی");
    });

    it("rejects addressId for guest checkout — guests have no saved addresses", async () => {
      const { service } = makeService();
      const dto = {
        paymentMethod: "CARD_TO_CARD" as const,
      ...
      await expect(service.checkout("cart-1", dto, null)).rejects.toBeInstanceOf(BadRequestException);
    });

    it("rejects checkout when neither addressId nor shippingAddress is provided", async () => {
      const { service } = makeService();
      const dto = {
        paymentMethod: "CARD_TO_CARD" as const,
        idempotencyKey: "44444444-4444-4444-4444-444444444444",
      };
      await expect(service.checkout("cart-1", dto, "customer-1")).rejects.toBeInstanceOf(BadRequestException);
    });

    it("propagates AddressesService's ownership error (e.g. ForbiddenException) rather than swallowing it", async () => {
      const { service, addresses } = makeService();
      (addresses.getOwned as jest.Mock).mockRejectedValue(new ForbiddenException("not yours"));
      const dto = {
        paymentMethod: "CARD_TO_CARD" as const,
        idempotencyKey: "55555555-5555-5555-5555-555555555555",
        addressId: "someone-elses-address",
      };
      await expect(service.checkout("cart-1", dto, "customer-1")).rejects.toBeInstanceOf(ForbiddenException);
    });
  });
  ...
  describe("cancelOrder", () => {
    ...
    it("restocks onHandGrams from order-item snapshots when cancelling an already-PAID order, since its reservations are already CONSUMED rather than ACTIVE", async () => {
      const { service, prisma, txMock } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest
        .fn()
        .mockResolvedValue({ id: "order-1", customerId: "customer-1", status: "PAID" });
      (txMock.orderItem.findMany as jest.Mock).mockResolvedValue([{ variantId: "variant-1", quantity: 2 }]);

      await service.cancelOrder("order-1", "customer-1");

      // 500g weight option x 2 quantity = 1000g restocked.
      expect(txMock.inventory.update).toHaveBeenCalledWith({
        where: { productProducerId: "pp-1" },
        data: { onHandGrams: { increment: 1000 } },
      });
      // The (now-irrelevant, already-CONSUMED) reservation path must not
      // also run — that would double-count stock changes for the same order.
      expect(txMock.inventoryReservation.findMany).not.toHaveBeenCalled();
    });
  });

  describe("adminSetStatus", () => {
    it("rejects an invalid transition (e.g. PENDING_PAYMENT straight to SHIPPED)", async () => {
      const { service, prisma } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest
        .fn()
        .mockResolvedValue({ id: "order-1", status: "PENDING_PAYMENT" });
      await expect(service.adminSetStatus("order-1", "SHIPPED", "admin-1")).rejects.toBeInstanceOf(
        BadRequestException,
      );
    });
    ...
    it("throws NotFoundException for a missing order", async () => {
      const { service, prisma } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest.fn().mockResolvedValue(null);
      await expect(service.adminSetStatus("missing", "SHIPPED", "admin-1")).rejects.toBeInstanceOf(NotFoundException);
    });

    it("advances PAID -> PROCESSING -> SHIPPED without touching inventory", async () => {
      const { service, prisma, txMock } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest
        .fn()
        .mockResolvedValue({ id: "order-1", status: "PAID" });

      await service.adminSetStatus("order-1", "PROCESSING", "admin-1", "بسته‌بندی شد");
      ...
      expect(txMock.orderStatusHistory.create).toHaveBeenCalledWith({
        data: { orderId: "order-1", status: "PROCESSING", changedByAdminId: "admin-1", note: "بسته‌بندی شد" },
      });
      expect(txMock.inventory.update).not.toHaveBeenCalled();
    });

    it("restocks inventory when an admin refunds a DELIVERED order", async () => {
      const { service, prisma, txMock } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest
        .fn()
        .mockResolvedValue({ id: "order-1", status: "DELIVERED" });
      (txMock.orderItem.findMany as jest.Mock).mockResolvedValue([{ variantId: "variant-1", quantity: 1 }]);

      await service.adminSetStatus("order-1", "REFUNDED", "admin-1");

      expect(txMock.inventory.update).toHaveBeenCalledWith({
        where: { productProducerId: "pp-1" },
        data: { onHandGrams: { increment: 500 } },
      });
      expect(txMock.order.update).toHaveBeenCalledWith({ where: { id: "order-1" }, data: { status: "REFUNDED" } });
    });

    it("skips a line whose variant was since deleted rather than crashing", async () => {
      const { service, prisma, txMock } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest
        .fn()
        .mockResolvedValue({ id: "order-1", status: "PAID" });
      (txMock.orderItem.findMany as jest.Mock).mockResolvedValue([{ variantId: "deleted-variant", quantity: 1 }]);
      (txMock.variant.findUnique as jest.Mock).mockResolvedValue(null);

      await expect(service.adminSetStatus("order-1", "CANCELLED", "admin-1")).resolves.toBeDefined();
      expect(txMock.inventory.update).not.toHaveBeenCalled();
    });
  });

  describe("verifyPayment", () => {
    it("reports manual methods as not verifiable through a provider", async () => {
      const { service, prisma } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest.fn().mockResolvedValue({
        id: "order-1",
        customerId: "customer-1",
        paymentAttempts: [{ method: "CARD_TO_CARD", providerRef: null }],
      });

      const result = await service.verifyPayment("order-1", "customer-1");
      expect(result.success).toBe(false);
    });

    it("throws a clear BadRequestException (not a generic 500) for the unconfigured ONLINE provider", async () => {
      const { service, prisma } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest.fn().mockResolvedValue({
        id: "order-1",
        customerId: "customer-1",
        paymentAttempts: [{ method: "ONLINE", providerRef: "ref-1" }],
      });

      await expect(service.verifyPayment("order-1", "customer-1")).rejects.toBeInstanceOf(BadRequestException);
    });

    it("throws NotFoundException when the order has no payment attempts at all", async () => {
      const { service, prisma } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest.fn().mockResolvedValue({
        id: "order-1",
        customerId: "customer-1",
      ...
      await expect(service.verifyPayment("order-1", "customer-1")).rejects.toBeInstanceOf(NotFoundException);
    });
  });

  describe("getForAdmin", () => {
    it("returns the order regardless of which customer owns it (no ownership check)", async () => {
      const { service, prisma } = makeService();
      (prisma.order as unknown as { findUnique: jest.Mock }).findUnique = jest.fn().mockResolvedValue({
        id: "order-1",
      ...
      const order = await service.getForAdmin("order-1");
      expect(order.id).toBe("order-1");
    });

    it("throws NotFoundException when the order doesn't exist", async () => {
      const { service } = makeService();
      await expect(service.getForAdmin("missing-order")).rejects.toBeInstanceOf(NotFoundException);
    });
  });

  describe("listForAdmin", () => {
    it("paginates and optionally filters by status", async () => {
      const findMany = jest.fn().mockResolvedValue([{ id: "order-1" }, { id: "order-2" }]);
      const count = jest.fn().mockResolvedValue(2);
      const { service } = makeService({
        prismaOverrides: {
          order: { findUnique: jest.fn(), findMany, count },
          $transaction: jest.fn((ops: Promise<unknown>[]) => Promise.all(ops)),
        },
      });

      const result = await service.listForAdmin({ page: 1, pageSize: 20, status: "PAID" });

      expect(result.total).toBe(2);
      expect(result.items).toHaveLength(2);
      expect(findMany).toHaveBeenCalledWith(
        expect.objectContaining({ where: { status: "PAID" }, skip: 0, take: 20 }),
      );
    });
  });
});
