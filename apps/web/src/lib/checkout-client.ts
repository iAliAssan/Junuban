...
export interface OrderView {
  ...
  items: OrderItemView[];
  paymentAttempts: OrderPaymentAttemptView[];
  customerNote: string | null;
}

export interface PaymentCardDisplay {
  number: string;
...
export interface CheckoutResponse {
  ...
  redirectUrl?: string;
  cardDisplay?: PaymentCardDisplay;
}
...
export const checkoutApi = {
  ...
  cancelOrder: (orderId: string) => apiFetch<OrderView>(`/orders/${orderId}/cancel`, { method: "POST" }),
  trackOrder: (orderNumber: string, phone: string) =>
    apiFetch<OrderView>("/orders/track", { method: "POST", body: JSON.stringify({ orderNumber, phone }) }),
};
