...
import { addressApi, type AddressView } from "@/lib/addresses-client";
...
import { PaymentCardDisplay } from "@/components/payment-card-display/payment-card-display";
...
export function CheckoutPageClient() {
  ...
  const [selectedAddressOption, setSelectedAddressOption] = useState<string>(NEW_ADDRESS_OPTION);
  ...
  useEffect(() => {
    ...
    addressApi
      ...
      .then((list) => {
        ...
        setSelectedAddressOption(preferred ? preferred.id : NEW_ADDRESS_OPTION);
      })
      .catch(() => {
        // Non-fatal — the customer can still enter an address inline.
        setSavedAddresses([]);
      })
  ...
  const usingNewAddress = selectedAddressOption === NEW_ADDRESS_OPTION || savedAddresses.length === 0;
  const cityOptions = citiesForProvince(address.province);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (submitting) return; // prevent double-submission from a double click

    const addressErrors = usingNewAddress ? validateShippingAddress(address) : {};
    ...
    try {
      const response = await checkoutApi.submit({
        shippingAddress: usingNewAddress ? address : undefined,
        addressId: usingNewAddress ? undefined : selectedAddressOption,
        paymentMethod,
      ...
      if (usingNewAddress && isAuthenticated && saveNewAddress) {
        // Best-effort — the order already succeeded regardless of this.
        addressApi.create(address).catch(() => {});
      }
    } catch (err) {
        ...
        <div className="container">
          <div className={styles.confirmation}>
            ...
            <div className={styles.confirmationItems}>
              {confirmed.order.items.map((item) => (
                <div key={item.id} className={styles.summaryItemGroup}>
                  <div className={styles.summaryItem}>
                    <span className={styles.summaryItemName}>
                      {item.productNameSnapshot} × {toPersianDigits(item.quantity)}
                    </span>
            ...
            {confirmed.cardDisplay ? (
              <PaymentCardDisplay card={confirmed.cardDisplay} />
            ) : (
            ...
            {isAuthenticated && (
              <Link href={`/orders/${confirmed.order.id}`} className={styles.continueLink}>
                مشاهده جزئیات سفارش ←
              </Link>
            )}
            <br />
        ...
        <form className={styles.grid} onSubmit={handleSubmit} noValidate>
          <div>
            ...
            <section className={styles.section}>
              ...
              {isAuthenticated && addressesLoaded && savedAddresses.length > 0 && (
                <div className={styles.savedAddressList} role="radiogroup" aria-label="انتخاب آدرس">
                  {savedAddresses.map((a) => (
                    <label key={a.id} className={styles.savedAddressOption}>
                      <input
                        type="radio"
                        name="addressChoice"
                        checked={selectedAddressOption === a.id}
                        onChange={() => setSelectedAddressOption(a.id)}
                      />
                      <span className={styles.savedAddressText}>
                        <strong>{a.recipientName}</strong> — {a.province}، {a.city}، {a.addressLine}
                        {a.isDefault && <span className={styles.defaultTag}> (پیش‌فرض)</span>}
                      </span>
                    </label>
                  ))}
                  <label className={styles.savedAddressOption}>
                    <input
                      type="radio"
                      name="addressChoice"
                      checked={selectedAddressOption === NEW_ADDRESS_OPTION}
                      onChange={() => setSelectedAddressOption(NEW_ADDRESS_OPTION)}
                    />
                    <span className={styles.savedAddressText}>آدرس جدید</span>
                  </label>
                </div>
              )}
                ...
                <div className={styles.fieldGrid}>
                  ...
                  <div className={styles.field}>
                    ...
                    >
                      ...
                      {address.province && !(IRAN_PROVINCES as readonly string[]).includes(address.province) && (
                  ...
                  {isAuthenticated && (
                    <div className={`${styles.field} ${styles.fieldFull} ${styles.checkboxField}`}>
                      <input
                        id="save-address"
                        type="checkbox"
                      ...
                      />
                      <label htmlFor="save-address">ذخیره این آدرس برای خریدهای بعدی</label>
                    </div>
                  )}
                </div>
              )}
            ...
            <section className={styles.section}>
              ...
              <div className={`${styles.field} ${styles.fieldFull}`}>
                <label htmlFor="customer-note">توضیحات (اختیاری)</label>
                <textarea
                  id="customer-note"
                  className={styles.noteTextarea}
                  rows={3}
                  maxLength={1000}
                  placeholder="اگر توضیحی درباره سفارش، بسته‌بندی یا تحویل دارید بنویسید..."
                  value={customerNote}
            ...
            {cart.items.map((item) => (
              <div key={item.id} className={styles.summaryItemGroup}>
                ...
                {item.packagingName && (
                  <div className={styles.summaryItem}>
                    <span className={styles.summaryItemName}>
                      {item.packagingName} × {toPersianDigits(item.quantity)}
                    </span>
                    <span>{formatToman(item.packagingPriceDelta * item.quantity)}</span>
                  </div>
                )}
              </div>
            ))}
