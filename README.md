...
**API + worker on a Linux VPS (Docker):** `apps/api/Dockerfile` and
`apps/worker/Dockerfile` build each as a normal long-running Node.js
process — no Vercel-specific code path is required to run them. Bring
up the full backend stack (Postgres, Redis, API, worker) with:
...
See the comments at the top of `docker-compose.prod.yml` for why this is
a separate file from the root `docker-compose.yml` (that one is
local-dev infrastructure only, per the Setup section above).

**API + worker without Docker:** each app's Dockerfile comment doubles
as the non-Docker instructions — in short, `npm run build` then `npm run
start` (or `node dist/main.js` / `node dist/index.js`) from each app's
directory, same as any Node.js server, with `DATABASE_URL`/`REDIS_URL`
pointed at real Postgres/Redis instances.

**API on Vercel:** `apps/api/src/main.ts` exports a serverless handler
that's used automatically when `process.env.VERCEL` is set; no separate
build path is needed.

**Web (storefront + admin):** a standard Next.js app — deploys to Vercel
directly, or anywhere else that runs `next build && next start`.
